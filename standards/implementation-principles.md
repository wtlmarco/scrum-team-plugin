...
Três passos, nesta ordem, antes do primeiro Plano de Implementação:
...
- `TODO`/`FIXME` só entra no merge **com o ID de uma Task aberta**; sem ID, bloqueia.
...
|---|---|
| *Boy Scout Rule* ("deixe o código melhor do que encontrou") | **Não vale como licença para refatorar de passagem.** O escopo é fechado pelo Plano de Implementação; melhoria percebida vira Task, registrada na seção "Não fiz (fora do plano)" do relatório do dev. Refatoração oportunista quebra a rastreabilidade entre Task, mudança e evidência |
...
| **Carga** | Só as operações declaradas sob orçamento de desempenho (§5.6, V18) | Reais, no ambiente de medição de V21 | Merge para a linha principal **e** em toda Task que toca uma operação sob orçamento |
...
**Como se verifica:** leitura do documento de requisitos antes do plano. RNF de performance incompleto **volta ao PO** e a Task não entra em construção (§7, #18).
...
**Como se verifica:** V18 preenchida (ou declarada vazia com o motivo) é condição de Ficha completa — sem Ficha, sem Plano de Implementação (§1).
...
| # | Task a vincular | O que preencher |
|---|---|---|
...
| 9 | `TODO`/`FIXME` sem ID de Task (§4.3) | Busca por padrão no pipeline | análise | **Bloqueia o merge** |
...
2. **Ligar formatter e linter** em modo verificação. É o passo mais barato e o que gera mais ruído — fazer isolado, numa Task própria, nunca junto de mudança funcional.
...
5. **Escrever o teste de arquitetura** (§2.3) com as violações atuais registradas como exceções nomeadas e datadas — cada exceção vira Task de backlog. Exceção sem Task é dívida invisível.
...
**Regra:** nenhum passo acima é feito "de passagem" dentro de uma Task de negócio. Cada um é uma Task própria, com plano, evidência e registro — pelo mesmo motivo de §4.5.
