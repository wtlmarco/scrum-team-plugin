...
5. **QA** — Agent `quality-assurance`, recebendo o plano, a especificação de tela (se houver), o relatório do dev e o critério de aceite do PO: seis frentes (requisito, especificação técnica — aderência de execução ao plano **e** de standard, as duas sempre —, segurança, testes/métricas, documentação, desempenho), execução real dos comandos de verificação do projeto, veredito ✅/⚠️/❌ endereçado ao stakeholder. Task com interface é validado também contra os seis estados e os critérios de acessibilidade da especificação.
6. Se o veredito for ⚠️ ou ❌, **não** siga para o fechamento — devolva os achados conforme a rota do próprio veredito: aderência de execução (código ≠ passo do plano) vai **direto** ao dev por `/dev resume <ID>`, sem passar pelo Arquiteto; defeito do plano (passo inconferível, seção de standard omitida ou errada) vai ao Arquiteto por 🔺 GAP/`/arc question` **e** entra como achado de processo na fila do `/review`; defeito do próprio standard vai só à fila do `/review`. `/arc comply` **não** roda neste ciclo — só como exceção pedida nomeadamente pelo stakeholder (`workflow.md` §4a). Se for ✅, informe que a Task está pronta para `/sm close <ID>` — o aceite é da História, agregado, e vem depois, na Sprint Review (R21).
