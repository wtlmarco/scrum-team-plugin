...
| Papel | Alcance |
|---|---|
| **Scrum Master** | `roles/scrum-master/` (roteiro, skills, modelos); **os normativos que governam todos** (`process/working-rules.md`, `process/workflow.md`, `process/artifact-ownership.md`); os entregáveis que possui — `deliverables/implementation/02-status.md` e os três índices transversais de curadoria (`deliverables/README.md`, `deliverables/implementation/README.md`, `deliverables/team-project/README.md` — `artifact-ownership.md` §1); a **curadoria** do processo do time inteiro — consolidar o changelog, apontar contradição entre mudanças de papéis diferentes, escalar o inconsistente |
...
| **UX** | `roles/user-experience/` — roteiro, skills e modelos (jornada, tela, revisão de usabilidade), incluindo os **seis estados** e a lista de critérios de acessibilidade verificáveis que vivem nesses modelos; o entregável que possui — `deliverables/prototype/`, que cobre **os três protótipos** e seus registros de verificação: o **funcional** (pré-condição do ①), o **do sprint** (pré-condição do ③ — R25) e o **de tela** (exploração no detalhamento) |
...
- **`/review metrics`** — revisão por evidência a partir dos indicadores do período, com **uma** proposta de mudança. Inclui o giro **Act** do ciclo de eficiência (`workflow.md` §5c): cada papel reporta **dois números separados** — a **carga fixa** (`agents/<papel>.md` + `commands/<papel>.md`, paga em toda invocação) e o **conjunto sob demanda** (`roles/<papel>/`, **sem os changelogs**, que são frios e crescem por decisão de R17). O SM consolida na tabela por papel, e a proposta única do período pode ser uma **remoção** — de preferência na carga fixa, onde o corte é multiplicado por toda invocação.
