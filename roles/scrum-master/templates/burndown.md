...
> **Dono:** SM · Vive em `.team-project/scrum-master/sprints/<n>/burndown.md`
> Nasce no `/sm sprint plan` (linha de abertura, dia 0), ganha uma linha a cada `/sm board` e a cada `/sm close <T-ID>` (R24), e **fecha** — sem mais edição — no `/sm sprint close`, junto com a retrospectiva e o snapshot do Sprint Backlog.
> **O que mede:** a estimativa restante (unidade do projeto) das Tasks ainda não fechadas (✅) do sprint corrente, em série datada — não o estado de cada Task, que já está no Sprint Backlog. **De onde sai o dado:** o Registro de transições de `.team-project/scrum-master/sprint-backlog.md` (R24). Este arquivo é a leitura em série daquele registro, não uma segunda fonte de verdade — se divergirem, o Registro de transições vence.
...
|---|---|---|---|---|---|---|---|---|---|---|
| 0 | <data> | Abertura do sprint (Planning) | <n> | <n> | <n> | 0 | 0 | 0 | 0 | 0 |
