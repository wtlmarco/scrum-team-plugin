...
| Registro de GAPs abertos | QA | Levantado sobre código; vence a narrativa de status — modelo em [`../../../deliverables/implementation/pending.md`](../../../deliverables/implementation/pending.md). **GAP não-bloqueante ganha par de linha no Product Backlog**, escrito pelo PO, no mesmo ciclo (R30 · [`workflow.md` §5e](workflow.md)) |
...
| **soma ou evolui através dos sprints** e seria quebrado se fatiado (registro de GAPs, mapa de código, **baseline de verificação**, **suíte de cenários de teste**, Product Backlog, SDD, ADRs, protótipo funcional do ①, **checkpoints de spike**, os `context.md`) | fora da pasta do sprint, no lugar que a matriz §1 já declara |
