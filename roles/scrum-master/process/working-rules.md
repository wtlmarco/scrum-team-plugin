...
| **Violação das regras de verificação binária** — R13 (gatilho de instrumento), R14 (onboarding), R15 (brainstorm, portões ① e ②), R16 (canal de defeito de standard), R18 (rastreabilidade da entrega), R19 (evidência do `/review`), R20 (Task↔História, fronteira funcional/técnica, índice sem conteúdo, construção antes do pacote), R21 (aceite na Review), R22 (forma e formulário da pergunta), R23 (verificação leve), R24 (transições e burndown), **R25** (pacote de abertura aprovado, `planning.md` com o que não entrou, protótipo que navega ponta a ponta, degrau do bloqueio nomeado, `stories/` congelado) | a linha **"SM verifica"** da própria regra, que nomeia o artefato a conferir | qualquer ocorrência → a regra foi ignorada. **Não há limiar a calibrar** — e é por isso que estas não ganham linha própria: repetir aqui a verificação que já está na regra era dizer duas vezes a mesma coisa (v3.23) |
