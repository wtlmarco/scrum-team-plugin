...
| Merge em `develop` sem bump de `version` + entrada no `CHANGELOG.md`, ou `plugin.json` ≠ topo do `CHANGELOG.md`, ou banner "Versão atual" de `README.md` ≠ `plugin.json`/topo do `CHANGELOG.md`, ou entrada de `process-changelog.md` sem par em `CHANGELOG.md` | `git log develop` + `CHANGELOG.md` + `plugin.json` + `README.md` | qualquer ocorrência → R18 ignorada (workflow §5d) |
